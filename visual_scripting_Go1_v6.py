...
# 1. Server Link Node (Data Provider)
class TargetIpNode(BaseNode):
    ...
    def execute(self):
        self.output_data[self.out_ip] = dpg.get_value(self.field_ip)
        return None
...
class CameraControlNode(BaseNode):
    ...
    def execute(self):
        ...
        # 핵심: 연결된 서버 IP가 없으면, PyGui가 돌아가는 현재 로컬 PC(라즈베리파이)의 IP를 자동으로 가져옵니다.
...
def execute_graph_once():
    ...
    # V6 Update: Add TargetIpNode to flowless execution, KEEP CameraControlNode OUT of this list (it needs Flow)
    for node in node_registry.values():
        if not isinstance(node, (StartNode, CommandActionNode, LogicIfNode, LogicLoopNode, ConditionCompareNode, ConditionKeyNode, PrintNode, KeyboardControlNode, RobotControlNode, CameraControlNode)):
            try: node.execute()
            except: pass
...
class NodeFactory:
    ...
    def create_node(node_type, node_id=None):
        ...
        elif node_type == "CAM_CTRL": node = CameraControlNode(node_id) # V6 Added
        elif node_type == "TARGET_IP": node = TargetIpNode(node_id)     # V6 Added
...
with dpg.window(tag="PrimaryWindow"):
    ...
    with dpg.group(horizontal=True):
        with dpg.child_window(width=250, height=130, border=True):
            dpg.add_text("System Status (V6)", color=(150,150,150))
    ...
    dpg.add_separator()
    # Tool Bar (V6: CAMERA Action & TARGET IP Data 분리)
    with dpg.group(horizontal=True):
        ...
        dpg.add_button(label="CAMERA (Ctrl)", callback=add_node_cb, user_data="CAM_CTRL")   # V6
        dpg.add_button(label="TARGET IP", callback=add_node_cb, user_data="TARGET_IP") # V6
...
dpg.create_viewport(title='PyGui Visual Scripting v6 (Modular Camera & Twin)', width=1024, height=768, vsync=True)
