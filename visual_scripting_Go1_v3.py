...
def execute_graph_once():
    ...
    # GraphNode, LoggerNode, GetStateNode, ConstantNode는 Flow 연결 없이 매 틱마다 데이터를 뿌리거나 수집해야 하므로 제외 목록에서 뺍니다.
    for node in node_registry.values():
...
# ================= [Main Setup] =================
threading.Thread(target=go1_v3_comm_thread, daemon=True).start()
...
with dpg.window(tag="PrimaryWindow"):
    ...
    with dpg.group(horizontal=True):
        with dpg.child_window(width=250, height=130, border=True):
            dpg.add_text("System Status (V3)", color=(150,150,150))
...
dpg.create_viewport(title='PyGui Visual Scripting v3 (Go1 Full Features)', width=1024, height=768, vsync=True)
