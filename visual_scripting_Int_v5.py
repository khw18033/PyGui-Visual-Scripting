...
# ★ [V5 추가] Direct Video Stream 노드 (파일 저장 없는 순수 스트리밍)
class DirectStreamNode(BaseNode):
    def __init__(self, node_id): 
        super().__init__(node_id, "Direct Video Stream", "DIRECT_STREAM")
        self.combo_action = None; self.out_flow = None; self.chk_aruco = None; self.input_size = None; self.field_url = None
    def build_ui(self):
        with dpg.node(tag=self.node_id, parent="node_editor", label="Direct Stream (No Save)"):
            ...
            with dpg.node_attribute(attribute_type=dpg.mvNode_Attr_Static): 
                ...
                dpg.add_text("Stream URL or Cam ID:")
                # Go1의 기본 mjpg-streamer 주소를 기본값으로 설정
                self.field_url = dpg.add_input_text(width=180, default_value="http://192.168.123.13:8080/?action=stream")
                dpg.add_spacer(height=3)
    ...
    def execute(self):
        ...
        if url.isdigit(): url = int(url) # 로컬 웹캠(0, 1 등) 처리
    ...
    def load_settings(self, data): dpg.set_value(self.combo_action, data.get("act", "Start Stream")); dpg.set_value(self.field_url, data.get("url", "http://192.168.123.13:8080/?action=stream")); dpg.set_value(self.chk_aruco, data.get("aruco", False)); dpg.set_value(self.input_size, data.get("size", 0.03))
