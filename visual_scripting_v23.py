...
FEEDBACK_PORT = 5005
LIMITS = {'min_x': 100, 'max_x': 280, 'min_y': -150, 'max_y': 150, 'min_z': 0, 'max_z': 280}
...
# ================= [Hardware Controller Core (V23 Architecture)] =================
class BaseRobotDriver(ABC):
    """
    [v23 아키텍처] 
    모든 로봇 드라이버가 상속받아야 하는 추상 클래스.
    GUI가 하드웨어를 어떻게 그려야 할지(Schema) 정보를 제공하고 실제 제어를 수행합니다.
    """
    @abstractmethod
    def get_ui_schema(self):
        """GUI 노드의 입력 핀 구성을 정의 반환 (Dict 형태)"""
        pass

    @abstractmethod
    def get_settings_schema(self):
        """GUI 노드의 세팅(파라미터) 핀 구성을 정의 반환"""
        pass

    @abstractmethod
    def execute_command(self, inputs, settings, current_pos, target_goal, manual_override_until, ser_conn):
        """실제 하드웨어 제어 및 상태 연산"""
        pass


class MT4RobotDriver(BaseRobotDriver):
    """MT4 로봇에 특화된 비즈니스 로직과 통신만 담당하는 클래스 (GUI 개입 없음)"""
    def __init__(self):
        self.last_cmd = ""
    ...
    def get_ui_schema(self):
        # UI에게 "나는 x, y, z, gripper 4개의 입력이 필요해" 라고 알려줌
        return {
            'x': ("X", 200.0),
            'y': ("Y", 0.0),
            'z': ("Z", 120.0),
    ...
    def execute_command(self, inputs, settings, current, target, override_until, ser_conn):
        # 1. 수동 제어가 아닐 때만 노드 입력값 적용
        if time.time() > override_until:
            for k in self.get_ui_schema().keys():
                if inputs.get(k) is not None:
                    target[k] = float(inputs[k])
        
        # 2. 스무딩 연산
        smooth = 1.0 if time.time() < override_until else max(0.01, min(settings.get('smooth', 1.0), 1.0))
        dx = target['x'] - current['x']; dy = target['y'] - current['y']; dz = target['z'] - current['z']
        
        nx = current['x'] + dx * smooth if not (abs(dx)<0.5 and abs(dy)<0.5 and abs(dz)<0.5) else target['x']
        ny = current['y'] + dy * smooth if not (abs(dx)<0.5 and abs(dy)<0.5 and abs(dz)<0.5) else target['y']
        nz = current['z'] + dz * smooth if not (abs(dx)<0.5 and abs(dy)<0.5 and abs(dz)<0.5) else target['z']
        ng = max(GRIPPER_MIN, min(target['gripper'], GRIPPER_MAX))
        
        # 3. 리미트 보정
        nx = max(LIMITS['min_x'], min(nx, LIMITS['max_x']))
        ny = max(LIMITS['min_y'], min(ny, LIMITS['max_y']))
        nz = max(LIMITS['min_z'], min(nz, LIMITS['max_z']))
...
# ================= [Dynamic Robot Node (Decorator/Wrapper)] =================
class UniversalRobotNode(BaseNode):
    """
    [v23 아키텍처] 
    모든 로봇 드라이버를 감쌀 수 있는 만능 껍데기(Decorator) 노드.
    로봇 드라이버 객체가 알려주는 Schema에 따라 UI를 동적으로 생성합니다.
    """
    def __init__(self, node_id, driver_instance):
        super().__init__(node_id, "Robot Driver", "ROBOT_CONTROL")
        self.driver = driver_instance # 주입받은 실제 로봇 드라이버 객체
    ...
    def build_ui(self):
        with dpg.node(tag=self.node_id, parent="node_editor", label=self.label):
            ...
            # 1. 드라이버가 요구하는 입력 핀들을 자동으로 반복 생성 (X, Y, Z 등 하드코딩 제거)
            for key, (label, default_val) in self.schema.items():
                with dpg.node_attribute(attribute_type=dpg.mvNode_Attr_Input) as aid:
                    with dpg.group(horizontal=True): 
                        ...
                        self.ui_fields[key] = dpg.add_input_float(width=80, default_value=default_val, step=0)
                    self.inputs[aid] = "Data"
                    self.in_pins[key] = aid
            
            with dpg.node_attribute(attribute_type=dpg.mvNode_Attr_Static): dpg.add_spacer(height=5) 
            
            # 2. 드라이버가 요구하는 세팅 핀들을 자동으로 반복 생성 (Smooth, Speed 등)
            for key, (label, default_val) in self.settings_schema.items():
                with dpg.node_attribute(attribute_type=dpg.mvNode_Attr_Input) as aid:
                    with dpg.group(horizontal=True): 
                        dpg.add_text(label)
                        self.setting_fields[key] = dpg.add_input_float(width=60, default_value=default_val, step=0)
                    self.inputs[aid] = "Data"
                    self.setting_pins[key] = aid
                    
            with dpg.node_attribute(attribute_type=dpg.mvNode_Attr_Output) as fout: 
                dpg.add_text("Flow Out"); self.outputs[fout]="Flow"

    def execute(self):
        global current_pos, target_goal, manual_override_until, ser
        
        # 1. 동적 입력값 수집
        fetched_inputs = {}
        for key, aid in self.in_pins.items():
            fetched_inputs[key] = self.fetch_input_data(aid)
        ...
        # 3. 내부의 실제 로봇 코어에 연산 및 제어 위임 (Delegation)
        new_state = self.driver.execute_command(
            fetched_inputs, fetched_settings, current_pos, target_goal, manual_override_until, ser
        )
        
        # 4. 연산 결과를 바탕으로 글로벌 상태와 UI 갱신
        if new_state:
            current_pos.update(new_state)
            for key in self.schema.keys():
                if key in new_state and abs(self.cache_ui[key] - new_state[key]) > 0.1:
                    dpg.set_value(self.ui_fields[key], new_state[key])
                    self.cache_ui[key] = new_state[key]
    ...
    def get_settings(self): 
        # 설정값 동적 저장
        return {k: dpg.get_value(v) for k, v in self.setting_fields.items()}
...
class UnityControlNode(BaseNode):
    ...
    def execute(self):
        if time.time() - dashboard_state.get("last_pkt_time", 0) > 0.5:
            self.output_data[self.out_x] = None; self.output_data[self.out_y] = None
            self.output_data[self.out_z] = None; self.output_data[self.out_g] = None
            return self.outputs
...
def execute_graph_once():
    ...
    for node in node_registry.values():
        if not isinstance(node, (StartNode, CommandActionNode, LogicIfNode, LogicLoopNode, UDPReceiverNode, KeyboardControlNode, UniversalRobotNode)):
            node.execute()
...
class NodeFactory:
    ...
    def create_node(node_type, node_id=None):
        ...
        # ★ [아키텍처 변경] 팩토리가 '만능 노드'를 만들 때 'MT4 클래스'를 주입(Dependency Injection)합니다.
        elif node_type == "ROBOT_CONTROL": node = UniversalRobotNode(node_id, MT4RobotDriver())
...
dpg.create_viewport(title='PyGui V23 (Polymorphic Decorator)', width=1024, height=768, vsync=True)
