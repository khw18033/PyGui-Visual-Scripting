...
# ----------------------------------------------------------------
    ...
    def execute(self):
        ...
        if is_new_input:
            # Quiz: Only log actionable detections (person, chair). Filter out rejected classes.
            actionable_objects = [obj for obj in near_objects if str(obj.get('policy', {}).get('action', '')).strip().lower() != 'observe']
            if actionable_objects:
                write_log(
                    f"[GO1 AUTO AVOID] near detections: {self._format_near_targets_for_log(actionable_objects)}"
                )
