...
class Go1RobotDriver(BaseRobotDriver):
    ...
    def execute_command(self, inputs, settings):
        ...
        # Go1의 실제 상태는 백그라운드 스레드에서 업데이트되므로 여기서는 None을 반환(UI 캐싱 무시)
        return None 
...
class MT4CommandActionNode(BaseNode):
    ...
    def execute(self):
        ...
        elif mode == "Homing": pass # 생략 (직접 구현 필요시 추가)
...
class Go1KeyboardNode(BaseNode):
    def __init__(self, node_id): super().__init__(node_id, "Keyboard (Go1)", "GO1_KEYBOARD"); self.out_vx = None; self.out_vy = None; self.out_wz = None
    def build_ui(self):
...
def execute_graph_once():
    ...
    # 1. Flow가 필요 없는 백그라운드 노드 자동 실행 (드라이버, 통신, 로그 등)
    # Action, If, Loop를 제외한 것들
    for node in node_registry.values():
...
with dpg.window(tag="PrimaryWindow"):
    # ★ [다형성 UI] Tab Bar를 이용해 두 로봇의 대시보드를 깔끔하게 분리!
    with dpg.tab_bar():
        ...
        with dpg.tab(label="🐕 Go1 Dashboard"):
            with dpg.group(horizontal=True):
                with dpg.child_window(width=250, height=140, border=True):
                    ...
                    dpg.add_text(f"Sender: Stopped", tag="go1_dash_sender", color=(200,200,200))
                    dpg.add_text("Reason: NONE", tag="go1_dash_reason")
        ...
        with dpg.tab(label="📁 Files & System"):
            with dpg.group(horizontal=True):
                with dpg.child_window(width=450, height=100, border=True):
                    dpg.add_text("Save:"); dpg.add_input_text(tag="file_name_input", default_value="my_graph", width=150); dpg.add_button(label="SAVE", callback=save_cb, width=60)
                    dpg.add_text("Load:"); dpg.add_combo(items=get_save_files(), tag="file_list_combo", width=150); dpg.add_button(label="LOAD", callback=load_cb, width=60)
...
dpg.create_viewport(title='PyGui V1 (MT4 + Go1 Integrated Framework)', width=1280, height=800, vsync=True)
...
while dpg.is_dearpygui_running():
    ...
    dpg.set_value("go1_dash_sender", f"Sender: {sender_state['status']}")
    if sender_state['status'] == 'Running': dpg.configure_item("go1_dash_sender", color=(0,255,0))
    else: dpg.configure_item("go1_dash_sender", color=(200,200,200))
