...
# ================= [MT4 Dashboard Callbacks (추가된 부분)] =================
...
with dpg.window(tag="PrimaryWindow"):
    # ★ [수정됨] 탭 이모지 제거, MT4 수동 제어 추가, 파일 관리자 가로 정렬
    with dpg.tab_bar():
        ...
        with dpg.tab(label="Files & System"):
            with dpg.group(horizontal=True):
                with dpg.child_window(width=650, height=100, border=True):
                    ...
                    # Save와 Load를 같은 가로선상(horizontal)으로 배치
                    with dpg.group(horizontal=True):
...
dpg.create_viewport(title='PyGui V2 (MT4 + Go1 Integrated Framework)', width=1280, height=800, vsync=True)
