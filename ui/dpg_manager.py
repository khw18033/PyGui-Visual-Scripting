...
class NodeUIRenderer:
    ...
    def sync_ui_to_state():
        ...
        for nid, node in node_registry.items():
            ...
            # GO1_AUTO_AVOIDANCE: escape 값은 정책 YAML에서 결정되므로 UI에서 변경하지 않음
                ...
                node.state['invert_waypoint_x'] = dpg.get_value(node.chk_invert_x)
                node.state['invert_waypoint_z'] = dpg.get_value(node.chk_invert_z)
    ...
    def sync_state_to_ui(node):
        ...
        # --- [Vision & Go1] State -> UI ---
            ...
            dpg.set_value(node.chk_invert_x, node.state.get('invert_waypoint_x', False))
            dpg.set_value(node.chk_invert_z, node.state.get('invert_waypoint_z', False))
    ...
    def _render_go1_unity_auto(node):
        with dpg.node(tag=node.node_id, parent="node_editor", label="Unity Autonomy (Go1)"):
            ...
            with dpg.node_attribute(attribute_type=dpg.mvNode_Attr_Static):
                ...
                node.chk_invert_x = dpg.add_checkbox(label="Invert Left/Right (Unity X)", default_value=False)
                node.chk_invert_z = dpg.add_checkbox(label="Invert Front/Back (Unity Z)", default_value=False)
