...
- 스크립트 실행 시 콘솔에서 Go1 본체/릴레이 IP를 확인하므로, 실행 터미널에서 입력 가능한 상태여야 합니다.
...
2. Go1 IP 확인/재입력
- 실행 직후 콘솔에서 Go1 IP를 묻는 프롬프트가 표시됩니다.
- 기본값은 현재 코드 설정값이며, 엔터를 누르면 기본값을 그대로 사용합니다.
- `현재 Go1 본체 IP가 x.x.x.x가 맞습니까? (y/n)` 질문에서 `y`를 입력하면 진행합니다.
- `n`을 입력하면 다시 IP를 입력받습니다.
- 입력한 값은 해당 실행 동안 아래 항목에 공통 적용됩니다.
	- Go1 SDK 통신 대상 IP
	- SSH Jump Host(`pi@<Go1_IP>`) 접속 대상
	- 라우팅 설정 게이트웨이(`192.168.123.0/24 via <Go1_IP>`)
...
2. 실행 중 IP 변경
- 현재 구조는 실행 시작 시 IP를 확정한 뒤 스레드/통신을 시작합니다.
- 실행 중에 IP를 바꾸려면 프로그램을 재시작해 새 IP를 다시 입력하는 방식을 권장합니다.
