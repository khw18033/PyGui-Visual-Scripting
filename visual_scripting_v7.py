...
# 설정값 (re_pi_dashboard_ctrl.py 기준)
UNITY_IP = "192.168.50.63" 
FEEDBACK_PORT = 5005
SMOOTHING_FACTOR = 0.2  # 원본과 동일하게 0.2로 설정
...
class UDPReceiverNode(BaseNode):
    def __init__(self, node_id):
        super().__init__(node_id, "UDP Receiver (Sync)")
    ...
    def execute(self):
        ...
        if not self.is_bound:
            try:
                ...
                self.is_bound = True
                print(f"[UDP] Bound to {port}")
            except Exception as e:
                if "Address already in use" in str(e): self.is_bound = True
                else: return self.outputs

        # 1. 수신 (Flush Buffer Strategy)
        # 버퍼에 쌓인 모든 데이터를 읽어서 최신값만 취함 (Lag 제거)
        latest_data = None
        try:
            while True:
        ...
        # 2. 송신 (Feedback Loop)
        # 원본 코드처럼 주기적으로 데이터를 보냄
        # Unity Connection 유지의 핵심
        try:
            # 원본 코드의 역변환 공식 적용 (Robot -> Unity)
            # x_unity = -y_robot / 1000
            # y_unity = z_robot / 1000
            # z_unity = x_robot / 1000
...
class RobotControlNode(BaseNode):
    def __init__(self, node_id):
        super().__init__(node_id, "MT4 Driver (Clone)")
...
with dpg.window(label="Visual Scripting V7 (Performance Clone)", width=1000, height=700):
...
dpg.create_viewport(title='PyGui Editor V7', width=1000, height=700)
dpg.setup_dearpygui()
dpg.show_viewport()
dpg.start_dearpygui()
dpg.destroy_context()