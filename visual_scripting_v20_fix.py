...
class CommandActionNode(BaseNode):
    ...
    def build_ui(self):
        with dpg.node(tag=self.node_id, parent="node_editor", label="Command Action"):
            ...
            with dpg.node_attribute(attribute_type=dpg.mvNode_Attr_Static):
                # ★ [기능 포함] Grip Relative (Add) 모드 추가됨
                self.combo_id = dpg.add_combo(items=["Move Relative (XYZ)", "Move Absolute (XYZ)", "Set Gripper (Abs)", "Grip Relative (Add)", "Homing"], default_value="Move Relative (XYZ)", width=150)
    ...
    def execute(self):
        ...
        elif mode.startswith("Grip Rel"): # ★ 그리퍼 더하기 로직 추가됨
            target_goal['gripper'] += v1; apply_limits_and_move()
        elif mode == "Homing":
            threading.Thread(target=homing_thread_func, daemon=True).start()
...
class NodeFactory:
    ...
    def create_node(node_type, node_id=None):
        ...
        node = None
        if node_type == "START": node = StartNode(node_id) # ★ START 수정됨
...
dpg.create_viewport(title='PyGui V20 (Final Ultimate)', width=1024, height=768, vsync=True)
