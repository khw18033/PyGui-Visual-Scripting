...
# ================= [★ V4 ArUco Vision Worker & Flask Server] =================
def go1_vision_worker_thread():
    ...
    while True:
        ...
        time.sleep(0.03) # 초당 약 30프레임 제한

if HAS_CV2_FLASK:
    @app.route('/')
    def index(): return "<h1>Go1 ArUco Vision Tracker (V4)</h1><img src='/video_feed' width='640'>"
    ...
    while True:
        if camera_command_queue:
            ...
            if cmd == 'START':
                ...
                for nano in nanos:
                    # ★ [수정됨] 전체 스크립트를 bash -c '...' 로 묶어서 한 번의 sudo로 완벽하게 뚫어냅니다.
...
# ★ [V4 업데이트] ArUco 인식 옵션이 추가된 카메라 노드
...
dpg.create_viewport(title='PyGui V4 (Vision ArUco Integrated)', width=1280, height=800, vsync=True)
...
while dpg.is_dearpygui_running():
    ...
    # ★ ArUco 상태 UI 업데이트
    if aruco_settings['enabled'] and HAS_CV2_FLASK: dpg.configure_item("go1_dash_aruco", default_value="ArUco: ON (Port 5000)", color=(0,255,255))
