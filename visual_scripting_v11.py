...
class RobotControlNode(BaseNode):
    ...
    def execute(self):
        ...
        # ★ 핵심 최적화: 값이 변했을 때만 UI 그리기 (CPU 부하 90% 감소)
...
class PrintNode(BaseNode):
    ...
    def execute(self):
        # 최적화: print()는 터미널 부하가 매우 큼. 값 변경 시에만 출력하거나 UI로 대체
        msg = f"{current_pos['gripper']:.1f}"
        if msg != self.last_msg:
             dpg.set_value(self.input_field, msg) # Print 대신 UI에 표시
             self.last_msg = msg
        return self.outputs
...
def run_loop():
    ...
    while is_running:
        ...
        # 루프 속도 조금 낮춤 (0.01 -> 0.03) UI 반응성 확보
        time.sleep(0.03)
