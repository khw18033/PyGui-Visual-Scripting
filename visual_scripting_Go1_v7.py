...
# ================= [Background Thread (Camera SSH V6/V7)] =================
...
# ================= [Background Thread (Multi Sender V7)] =================
def cleanup_worker(config):
    global multi_sender_active
    folder = config["folder"]
    write_log(f"Cleaner: Started for {folder}")
...
async def send_image_async(session, filepath, camera_id, server_url):
    try:
        ...
        form.add_field('file', file_data, filename=os.path.basename(filepath), content_type='image/jpeg')
        async with session.post(server_url, data=form, timeout=2.0) as response:
        ...
        if sender_command_queue:
            ...
            if cmd == 'START' and not multi_sender_active:
                ...
                for config in CAMERA_CONFIG:
                    c_thread = threading.Thread(target=cleanup_worker, args=(config,))
                    c_thread.daemon = True
                    c_thread.start()
                    s_thread = threading.Thread(target=start_async_loop, args=(config, url))
...
# ================= [V7 AI Server Sender Node] =================
class MultiSenderNode(BaseNode):
    def __init__(self, node_id):
        super().__init__(node_id, "AI Server Sender", "MULTI_SENDER")
        self.combo_action = None; self.field_url = None; self.out_flow = None
        
    def build_ui(self):
        with dpg.node(tag=self.node_id, parent="node_editor", label="AI Server Sender (FastAPI)"):
            ...
            with dpg.node_attribute(attribute_type=dpg.mvNode_Attr_Static):
                self.combo_action = dpg.add_combo(["Start Sender", "Stop Sender"], default_value="Start Sender", width=140)
                dpg.add_spacer(height=3)
                dpg.add_text("Server URL:")
                self.field_url = dpg.add_input_text(width=160, default_value="http://210.110.250.33:5001/upload")
    ...
    def execute(self):
        ...
        url = dpg.get_value(self.field_url)
    ...
    def load_settings(self, data): 
        dpg.set_value(self.combo_action, data.get("act", "Start Sender"))
        dpg.set_value(self.field_url, data.get("url", "http://210.110.250.33:5001/upload"))

# ================= [V6 Separated Camera Nodes] =================
class TargetIpNode(BaseNode):
    def __init__(self, node_id):
        super().__init__(node_id, "Target IP Config", "TARGET_IP")
        self.field_ip = None; self.out_ip = None
    def build_ui(self):
        with dpg.node(tag=self.node_id, parent="node_editor", label="Target IP (Server)"):
            with dpg.node_attribute(attribute_type=dpg.mvNode_Attr_Static):
                dpg.add_text("Stream To PC IP:")
                self.field_ip = dpg.add_input_text(width=120, default_value="192.168.50.81")
            with dpg.node_attribute(attribute_type=dpg.mvNode_Attr_Output) as out:
                dpg.add_text("IP String"); self.outputs[out] = "Data"; self.out_ip = out
    def execute(self):
        self.output_data[self.out_ip] = dpg.get_value(self.field_ip); return None
    def get_settings(self): return {"ip": dpg.get_value(self.field_ip)}
    def load_settings(self, data): dpg.set_value(self.field_ip, data.get("ip", "192.168.50.81"))

class CameraControlNode(BaseNode):
    def __init__(self, node_id):
        super().__init__(node_id, "Camera Control", "CAM_CTRL")
        self.combo_action = None; self.in_ip = None; self.out_flow = None
    def build_ui(self):
        with dpg.node(tag=self.node_id, parent="node_editor", label="Camera Control"):
            with dpg.node_attribute(attribute_type=dpg.mvNode_Attr_Input) as flow: 
                dpg.add_text("Flow In"); self.inputs[flow] = "Flow"
            with dpg.node_attribute(attribute_type=dpg.mvNode_Attr_Static):
                self.combo_action = dpg.add_combo(["Start Stream", "Stop Stream"], default_value="Start Stream", width=120)
                dpg.add_spacer(height=3)
            with dpg.node_attribute(attribute_type=dpg.mvNode_Attr_Input) as ip_in:
                dpg.add_text("Target IP In", color=(255,150,200))
                self.inputs[ip_in] = "Data"; self.in_ip = ip_in
            with dpg.node_attribute(attribute_type=dpg.mvNode_Attr_Output) as out: 
                dpg.add_text("Flow Out"); self.outputs[out] = "Flow"; self.out_flow = out
    def execute(self):
        action = dpg.get_value(self.combo_action)
        ext_ip = self.fetch_input_data(self.in_ip)
        target_ip = ext_ip if ext_ip else get_local_ip()
...
def execute_graph_once():
    ...
    # 제외 목록에 MultiSenderNode 추가 (Flow로 실행되게 함)
    for node in node_registry.values():
        if not isinstance(node, (StartNode, CommandActionNode, LogicIfNode, LogicLoopNode, ConditionCompareNode, ConditionKeyNode, PrintNode, KeyboardControlNode, RobotControlNode, CameraControlNode, MultiSenderNode)):
            try: node.execute()
            except: pass
...
class NodeFactory:
    ...
    def create_node(node_type, node_id=None):
        ...
        elif node_type == "MULTI_SENDER": node = MultiSenderNode(node_id) # V7 새로 추가됨
...
threading.Thread(target=sender_manager_thread, daemon=True).start() # V7 Multi Sender Thread
...
with dpg.window(tag="PrimaryWindow"):
    ...
    with dpg.group(horizontal=True):
        with dpg.child_window(width=250, height=140, border=True):
            dpg.add_text("System Status (V7)", color=(150,150,150))
            ...
            dpg.add_text(f"Camera: Stopped", tag="dash_cam", color=(200,200,200))
            dpg.add_text(f"Sender: Stopped", tag="dash_sender", color=(200,200,200)) # V7 Sender UI 추가
    ...
    with dpg.group(horizontal=True):
        ...
        dpg.add_button(label="CAMERA (Ctrl)", callback=add_node_cb, user_data="CAM_CTRL")
        dpg.add_button(label="TARGET IP", callback=add_node_cb, user_data="TARGET_IP")
        dpg.add_button(label="SERVER SENDER", callback=add_node_cb, user_data="MULTI_SENDER") # V7 추가됨
...
dpg.create_viewport(title='PyGui Visual Scripting v7 (Modular Camera, AI Sender & Twin)', width=1080, height=768, vsync=True)
...
while dpg.is_dearpygui_running():
    ...
    dpg.set_value("dash_sender", f"Sender: {sender_state['status']}")
    if sender_state['status'] == 'Running': dpg.configure_item("dash_sender", color=(0,255,0))
    else: dpg.configure_item("dash_sender", color=(200,200,200))
