...
FEEDBACK_PORT = 5005
# ★ [버그 픽스] 오프셋이 더해져도 한계에 부딪히지 않도록 max_z를 280으로 넉넉히 올림
...
# ================= [Hardware Controller Core (Refactored)] =================
class MT4HardwareCore:
    """
    [v22 리팩토링 적용]
    GUI(DearPyGUI) 컴포넌트와 완전히 분리된 하드웨어 통신 및 위치 연산 전담 코어 클래스입니다.
    데코레이터(래핑) 패턴 적용을 위해 순수 비즈니스 로직만 포함합니다.
    """
    def __init__(self):
        self.last_cmd = ""
        self.last_write_time = 0
        self.write_interval = 0.0  # 통신 병목 해제 (프레임 드랍 방지)

    def calculate_smoothed_position(self, target, current, smooth_factor):
        """부드러운 이동(Smoothing)을 위한 다음 1프레임 좌표 연산"""
        dx = target['x'] - current['x']
        dy = target['y'] - current['y']
        dz = target['z'] - current['z']
        
        if abs(dx) < 0.5 and abs(dy) < 0.5 and abs(dz) < 0.5:
            nx, ny, nz = target['x'], target['y'], target['z']
        else:
            nx = current['x'] + dx * smooth_factor
            ny = current['y'] + dy * smooth_factor
            nz = current['z'] + dz * smooth_factor
    ...
    def send_gcode(self, serial_conn, nx, ny, nz, ng):
        """계산된 좌표를 G-Code 문자열로 변환하여 실제 시리얼 포트로 전송"""
...
class RobotControlNode(BaseNode):
    def __init__(self, node_id):
        ...
        self.cache_ui={'x':0,'y':0,'z':0,'g':0}
        
        # ★ [v22 리팩토링 핵심] 데코레이터 패턴 도입. GUI 노드가 코어 연산 객체를 포함(Wrapping)합니다.
        self.hw_core = MT4HardwareCore()
    ...
    def execute(self):
        ...
        # ★ 2. 분리된 코어 비즈니스 로직에 연산 위임 (로직과 UI의 분리)
        nx, ny, nz, ng = self.hw_core.calculate_smoothed_position(target_goal, current_pos, smooth)
        
        current_pos.update({'x':nx, 'y':ny, 'z':nz, 'gripper':ng})
        
        # ★ 3. 코어가 연산한 상태를 읽어와 화면(GUI)에 표시
        ...
        # ★ 4. 분리된 코어에 하드웨어 전송 위임
        self.hw_core.send_gcode(ser, nx, ny, nz, ng)
    ...
    def load_settings(self, data): dpg.set_value(self.field_smooth, data.get("s", 1.0)); dpg.set_value(self.field_g_speed, data.get("sp", 2.0))
...
class UnityControlNode(BaseNode):
    ...
    def execute(self):
        # ★ [버그 픽스] 1. 유니티 패킷이 끊긴 지 0.5초가 넘었는지 확인 (Idle 상태 감지)
        if time.time() - dashboard_state.get("last_pkt_time", 0) > 0.5:
            # 유니티 조작이 멈추면 파이썬/Direct 제어에 권한을 양보
...
dpg.create_viewport(title='PyGui V22 (Refactored)', width=1024, height=768, vsync=True)
dpg.setup_dearpygui(); dpg.set_primary_window("PrimaryWindow", True); dpg.show_viewport()

last_logic_time = 0
LOGIC_RATE = 0.05 # 20 FPS (프레임 엇박자 해결 유지)
