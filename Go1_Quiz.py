...
def go1_v4_comm_thread():
    ...
    control_delay_buffer = deque(maxlen=4000)
    delayed_vx = delayed_vy = delayed_wz = 0.0
    ...
    while True:
        ...
        while control_delay_buffer and (tnow - control_delay_buffer[0][0]) >= CONTROL_INPUT_DELAY_SEC:
            latest_ready = control_delay_buffer.popleft()
        if latest_ready is not None:
            delayed_vx, delayed_vy, delayed_wz = latest_ready[1], latest_ready[2], latest_ready[3]
        ...
        if yaw_align_active:
            ...
            target_mode = 2
            if cmd: cmd.gaitType = 1
            out_vx = clamp(delayed_vx, -V_MAX, V_MAX); out_vy = clamp(delayed_vy, -S_MAX, S_MAX); out_wz = clamp(delayed_wz, -W_MAX, W_MAX)
            go1_state['reason'] = "NODE_WALK_DELAYED"
        else:
