...
# 네트워크 실시간 감지 스레드 (다중 네트워크 연결 확인용)
...
class MT4RobotDriver(BaseRobotDriver):
    ...
    def execute_command(self, inputs, settings):
        ...
        if time.time() < mt4_collision_lock_until: return # 충돌 락 방어 로직
...
def execute_graph_once():
    ...
    for node in node_registry.values():
        if not isinstance(node, (StartNode, MT4CommandActionNode, LogicIfNode, LogicLoopNode, ConditionKeyNode)):
            try: node.execute()
            except: pass
...
with dpg.window(tag="PrimaryWindow"):
    with dpg.tab_bar():
        ...
        # 두 번째 탭: Files & System (대시보드 분리 및 다중 네트워크 모니터)
...
dpg.create_viewport(title='MT4 Educational V24', width=1280, height=800, vsync=True)
