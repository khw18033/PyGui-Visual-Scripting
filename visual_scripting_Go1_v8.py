...
# V8 Multi Sender Variables (from multi_sender.py)
...
# ================= [Background Thread (Camera SSH & Local Recv V8)] =================
    ...
    while True:
        if camera_command_queue:
            ...
            if cmd == 'START':
                ...
                # 2. 로컬 이미지 수신기 (pc_recv_all.sh 역할) 파이썬 내부 실행
                # 기존 수신기 청소
                subprocess.call("pkill -f 'gst-launch-1.0.*multifilesink'", shell=True)
                time.sleep(0.5)
                
                # 포트 및 저장 경로 매핑 (RAM Disk: /dev/shm)
                recv_configs = [
                ...
                write_log("Cam: Starting local receivers to /dev/shm/")
...
# ================= [Background Thread (Multi Sender V8)] =================
        ...
        if sender_command_queue:
            ...
            if cmd == 'START' and not multi_sender_active:
                ...
                for config in CAMERA_CONFIG:
                    ...
                    c_thread.daemon = True; c_thread.start()
                    s_thread = threading.Thread(target=start_async_loop, args=(config, url))
                    s_thread.daemon = True; s_thread.start()
...
def go1_v4_comm_thread():
    ...
    while True:
        ...
        if got: 
            ...
            unity_teleop_data['vx'] = uvx; unity_teleop_data['vy'] = uvy
            unity_teleop_data['wz'] = uwz; unity_teleop_data['estop'] = uestop
    ...
    def build_ui(self):
        with dpg.node(tag=self.node_id, parent="node_editor", label="Target IP (Receiver)"):
            with dpg.node_attribute(attribute_type=dpg.mvNode_Attr_Static):
                dpg.add_text("Save Data To IP:")
                self.field_ip = dpg.add_input_text(width=120, default_value=get_local_ip())
...
class CameraControlNode(BaseNode):
    ...
    def build_ui(self):
        with dpg.node(tag=self.node_id, parent="node_editor", label="Camera Control (V8)"):
...
class MultiSenderNode(BaseNode):
    ...
    def build_ui(self):
        with dpg.node(tag=self.node_id, parent="node_editor", label="AI Server Sender (V8)"):
            with dpg.node_attribute(attribute_type=dpg.mvNode_Attr_Input) as flow: dpg.add_text("Flow In"); self.inputs[flow] = "Flow"
            with dpg.node_attribute(attribute_type=dpg.mvNode_Attr_Static):
                self.combo_action = dpg.add_combo(["Start Sender", "Stop Sender"], default_value="Start Sender", width=140)
                dpg.add_spacer(height=3)
                dpg.add_text("AI Server URL:")
...
def execute_graph_once():
    ...
    # Flow 노드 외의 백그라운드 노드들은 매 틱 자동 실행
    for node in node_registry.values():
...
with dpg.window(tag="PrimaryWindow"):
    ...
    with dpg.group(horizontal=True):
        with dpg.child_window(width=250, height=140, border=True):
            dpg.add_text("System Status (V8)", color=(150,150,150))
            ...
            dpg.add_text(f"Sender: Stopped", tag="dash_sender", color=(200,200,200))
    ...
    with dpg.group(horizontal=True):
        ...
        dpg.add_button(label="SERVER SENDER", callback=add_node_cb, user_data="MULTI_SENDER")
...
dpg.create_viewport(title='PyGui Visual Scripting v8 (Auto RAM-Disk Receiver & Sender)', width=1080, height=768, vsync=True)
