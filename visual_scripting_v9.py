...
FEEDBACK_PORT = 5005
SMOOTHING_FACTOR = 0.2  # XYZ Movement Smoothing
LIMITS = {'min_x': 100, 'max_x': 280, 'min_y': -150, 'max_y': 150, 'min_z': 0, 'max_z': 180}

# ★ [핵심 설정] 그리퍼 속도 조절 (낮을수록 천천히 움직임)
# 루프당 0.5도씩 움직임 -> 40에서 60까지 약 0.5~1초 소요
GRIPPER_SPEED = 0.5 
...
class RobotControlNode(BaseNode):
    ...
    def execute(self):
        ...
        # 3. ★ [핵심 변경] Gripper Smoothing (그리퍼 부드럽게 이동)
        dg = target_goal['gripper'] - current_pos['gripper']
        
        # 차이가 작으면 바로 도달
        if abs(dg) < GRIPPER_SPEED:
            next_g = target_goal['gripper']
        else:
            # 목표 방향으로 조금씩 이동 (40 -> 40.5 -> 41 ...)
            direction = 1.0 if dg > 0 else -1.0
            next_g = current_pos['gripper'] + (direction * GRIPPER_SPEED)

        # 상태 업데이트
        current_pos['x'] = next_x
        current_pos['y'] = next_y
        current_pos['z'] = next_z
        current_pos['gripper'] = next_g # 스무딩된 값 적용
...
dpg.create_context()
# Lite Version (No Fonts)

with dpg.handler_registry():
    dpg.add_key_press_handler(dpg.mvKey_Delete, callback=delete_selection)

with dpg.window(label="Visual Scripting V9 (Gripper Smooth)", width=1000, height=700):
...
dpg.create_viewport(title='PyGui V9 (Gripper Fix)', width=1000, height=700, vsync=False)
dpg.setup_dearpygui()
dpg.show_viewport()
dpg.start_dearpygui()
dpg.destroy_context()