...
def manual_control_callback(sender, app_data, user_data):
    ...
    if axis == 'gripper':
        # 그리퍼 토글 (40 <-> 60)
        # 현재 상태가 열림(40)이면 닫음(60), 아니면 열림
        val = 60.0 if target_goal['gripper'] is None or target_goal['gripper'] < 50 else 40.0
...
def move_to_coord_callback(sender, app_data, user_data):
    ...
    target_goal['x'] = float(x)
    target_goal['y'] = float(y)
    target_goal['z'] = float(z)
    target_goal['gripper'] = float(g)
...
def homing_callback(sender, app_data, user_data):
    # 호밍은 시간이 오래 걸리므로 별도 스레드 처리 (UI 멈춤 방지)
...
class RobotControlNode(BaseNode):
    ...
    def build_ui(self):
        with dpg.node(tag=self.node_id, parent="node_editor", label=self.label):
            ...
            # ★ V13 Fix: 구분선을 안전하게 속성 안에 포함
            with dpg.node_attribute(attribute_type=dpg.mvNode_Attr_Static):
                dpg.add_separator()

            # ★ V13 Feature: Speed Control Inputs
    ...
    def execute(self):
        ...
        # 3. 목표값 업데이트 (연결된 노드가 없으면 대시보드 조작값 유지)
...
dpg.create_viewport(title='PyGui V14 (Integrated Dashboard)', width=1024, height=768, vsync=True)
dpg.setup_dearpygui()
