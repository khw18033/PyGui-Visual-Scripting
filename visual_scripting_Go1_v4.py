...
unity_teleop_data = {'vx': 0.0, 'vy': 0.0, 'wz': 0.0, 'estop': 0, 'active': False} # V4 New
...
class UnityControlNode(BaseNode):
    ...
    def build_ui(self):
        with dpg.node(tag=self.node_id, parent="node_editor", label="Unity Link (Flowless)"):
            ...
            with dpg.node_attribute(attribute_type=dpg.mvNode_Attr_Output) as vx: 
                dpg.add_text("Teleop Vx"); self.outputs[vx] = "Data"; self.out_vx = vx
            with dpg.node_attribute(attribute_type=dpg.mvNode_Attr_Output) as vy: 
                dpg.add_text("Teleop Vy"); self.outputs[vy] = "Data"; self.out_vy = vy
            with dpg.node_attribute(attribute_type=dpg.mvNode_Attr_Output) as wz: 
                dpg.add_text("Teleop Wz"); self.outputs[wz] = "Data"; self.out_wz = wz
            with dpg.node_attribute(attribute_type=dpg.mvNode_Attr_Output) as act: 
                dpg.add_text("Is Active?"); self.outputs[act] = "Data"; self.out_active = act
                
    def execute(self):
        global UNITY_IP, node_intent
        # Update dynamically without restarting script
        UNITY_IP = dpg.get_value(self.field_ip)
        node_intent['use_unity_cmd'] = dpg.get_value(self.chk_enable)
        
        # Provide received data to graph if user wants to use it
        self.output_data[self.out_vx] = unity_teleop_data['vx']
        self.output_data[self.out_vy] = unity_teleop_data['vy']
        self.output_data[self.out_wz] = unity_teleop_data['wz']
        self.output_data[self.out_active] = unity_teleop_data['active']
        return None
...
class CommandActionNode(BaseNode):
    ...
    def execute(self):
        ...
        if mode == "Stand": node_intent['stop'] = True
            ...
            node_intent['vx'] = v1 if mode == "Walk Fwd/Back" else 0.0
            node_intent['vy'] = v1 if mode == "Walk Strafe" else 0.0
            node_intent['wz'] = v1 if mode == "Turn" else 0.0
...
class GraphNode(BaseNode):
            ...
            with dpg.node_attribute(attribute_type=dpg.mvNode_Attr_Static):
                with dpg.plot(height=150, width=250):
                    ...
                    with dpg.plot_axis(dpg.mvYAxis, label="Val", tag=f"ay_{self.node_id}"):
                        dpg.add_line_series([],[], label="1", tag=f"sx_{self.node_id}")
    ...
    def execute(self):
        ...
        if vx is not None or vy is not None or vz is not None:
            ...
            dpg.set_value(f"sy_{self.node_id}", [list(self.t), list(self.buf_y)])
            dpg.set_value(f"sz_{self.node_id}", [list(self.t), list(self.buf_z)])
...
class GetStateNode(BaseNode):
    ...
    def execute(self):
        self.output_data[self.out_x] = go1_state['world_x']; self.output_data[self.out_z] = go1_state['world_z']; self.output_data[self.out_yaw] = go1_state['yaw_unity']
        return None
...
def execute_graph_once():
    ...
    # GraphNode, LoggerNode, GetStateNode, ConstantNode, UnityControlNode는 Flow 연결 없이 매 틱마다 데이터를 뿌리거나 수집해야 하므로 제외 목록에서 뺍니다.
    for node in node_registry.values():
        if not isinstance(node, (StartNode, CommandActionNode, LogicIfNode, LogicLoopNode, ConditionCompareNode, ConditionKeyNode, PrintNode, KeyboardControlNode, RobotControlNode)):
            try: node.execute()
            except: pass
...
with dpg.window(tag="PrimaryWindow"):
    ...
    with dpg.group(horizontal=True):
        with dpg.child_window(width=250, height=130, border=True):
            dpg.add_text("System Status (V4)", color=(150,150,150))
    ...
    with dpg.group(horizontal=True):
        ...
        dpg.add_button(label="UNITY", callback=add_node_cb, user_data="UNITY_CONTROL") # V4 추가
...
dpg.create_viewport(title='PyGui Visual Scripting v4 (Unity Digital Twin)', width=1024, height=768, vsync=True)
